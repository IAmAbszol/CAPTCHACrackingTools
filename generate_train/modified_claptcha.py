...
class Claptcha(object):
    ...
    def image(self):
        ...
        # Resize
        # Disable resizing on module to keep image relatively small for training 
        # image = image.resize((400, 160), resample=self.resample)

        # Text
        offsets = self._writeText(image, text, pos=(margin_x, margin_y))
    ...
    def _writeText(self, image, text, pos):
        ...
        for c in text:
            ...
            widths.append((c_image.size[0]))
            heights.append((c_image.size[1]))
            offset += c_size[0]

        return [offsets, y, widths, heights]
