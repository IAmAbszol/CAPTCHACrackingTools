...
class _Captcha(object):
    def generate(self, chars, format='png'):
        ...
        """
        im, offsets = self.generate_image(chars)
        out = BytesIO()
...
class ImageCaptcha(_Captcha):
    ...
    def create_captcha_image(self, chars, color, background):
        ...
        for c in chars:
            if random.random() > 0.5:
                images.append((_draw_character(" "), 0))
            images.append((_draw_character(c), 1))

        text_width = sum([im.size[0] for im, c in images])
    ...
    def generate_image(self, chars):
        ...
        im, offsets = self.create_captcha_image(chars, color, background)
